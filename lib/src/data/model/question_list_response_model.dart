class QuestionListResponseModel {
  int? status;
  String? message;
  List<QuestionListDataModel>? data;

  QuestionListResponseModel({this.status, this.message, this.data});

  QuestionListResponseModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
      data = <QuestionListDataModel>[];
      json['data'].forEach((v) {
        data!.add(QuestionListDataModel.fromJson(v));
      });
    }
  }
